...
- **Request type changes:** `from_address`/`to_address` → `from`/`to`; weight and dimensions extracted into a `parcel` object; `package_count` removed (single parcel at MVP); `weight_unit: 'lbs'` → `'lb'` to match provider APIs.
